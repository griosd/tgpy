...
class Norm2Transport(TgTransport):
    """
    y = Q(F(|x|)) x / |x|
    The Norm2Transport class is a subclass of TgTransport and represents a transport map between probability
    distributions using the L2 norm.
    It is used in the context of normalizing flows and can be used to transform a standard normal distribution to an
    arbitrary distribution specified by the user.
    """
    def __init__(self, obj, *args, **kwargs):
        """
        Initializes a new instance of Norm2Transport.

        Parameters:
        obj: an instance of nn.Module representing the distribution to be transformed.
        *args: optional positional arguments for the TgTransport constructor.
        **kwargs: optional keyword arguments for the TgTransport constructor.
        """
    ...
    def norm_y(self, norm_x, n=None):
        """
        Computes the normalized output distribution of the transport map given the normalized input distribution.

        Parameters:
        norm_x: a tensor representing the normalized input distribution.
        n: an integer representing the number of time steps (default: None).

        Returns:
        a tensor representing the normalized output distribution.
        """
        return self.obj.Q(self.ref.F(norm_x, n), n)

    def norm_x(self, norm_y, n=None):
        """
        Computes the normalized input distribution of the transport map given the normalized output distribution.

        Parameters:

        norm_y: a tensor representing the normalized output distribution.
        n: an integer representing the number of time steps (default: None).

        Returns:
        a tensor representing the normalized input distribution.
        """
        return self.ref.Q(self.obj.F(norm_y, n), n)

    def forward(self, t, x, noise=None):
        """
        Transforms a sample from the input distribution to a sample from the output distribution.

        Parameters:
        t: a tensor representing the time step.
        x: a tensor representing a sample from the input distribution.
        noise: optional noise tensor (default: None).

        Returns:
        a tensor representing a sample from the output distribution.
        """
        normx = x.norm(dim=-2)
    ...
    def inverse(self, t, y, noise=None):
        """
        Transforms a sample from the output distribution to a sample from the input distribution.}

        Parameters:
        t: a tensor representing the time step.
        y: a tensor representing a sample from the output distribution.
        noise: optional noise tensor (default: None).

        Returns:
        a tensor representing a sample from the input distribution.
        """
    ...
    def posterior_norm_y(self, norm_x, n=None, n_obs=None, norm_y_obs=None):
        """
        Computes the posterior distribution of the normalized output distribution given the normalized input
        distribution and observed data.

        Parameters:
        norm_x: a tensor representing the normalized input distribution.
        n: an integer representing the number of time steps (default: None).
        n_obs: an integer representing the number of observed time steps (default: None).
        norm_y_obs: a tensor representing the normalized observed data (default: None).

        Returns:
        a tensor representing the posterior distribution of the normalized output distribution.
        """
    ...
    def posterior(self, t, x, obs_t, obs_y, generator=None, noise=False, *args, **kwargs):
        """
        Computes the posterior distribution of the output distribution given the input distribution and observed data.

        Parameters:
        t: a tensor representing the time step.
        x: a tensor representing a sample from the input distribution.
        obs_t: a tensor representing the observed
        """
        normx = x.norm(dim=-2)
